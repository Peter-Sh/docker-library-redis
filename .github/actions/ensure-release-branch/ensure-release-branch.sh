...
if echo "$last_cmd_stdout" | grep -q "$RELEASE_BRANCH"; then
    ...
    execute_command git fetch --unshallow origin "$RELEASE_BRANCH" || execute_command git fetch origin "$RELEASE_BRANCH"
else
    ...
    execute_command git fetch --unshallow origin "$BASE_BRANCH" || execute_command git fetch origin "$BASE_BRANCH"
...
if echo "$last_cmd_stdout" | grep -q "$RELEASE_VERSION_BRANCH"; then
    execute_command git fetch --unshallow origin "$RELEASE_VERSION_BRANCH" || execute_command git fetch origin "$RELEASE_VERSION_BRANCH"
